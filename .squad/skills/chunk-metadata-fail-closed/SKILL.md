...
- Treat duplicated identity fields in persisted or remote download metadata (for example duplicate manifest `fileId` values) as invalid metadata, not as a "pick one" convenience case.
...
- `src/ShadowDrop.Cli/Downloads/DownloadCommandHandler.cs`: file selection rejects duplicate manifest `fileId` values with `DownloadCommandException` so malformed manifests do not escape as framework exceptions.
- `tests/ShadowDrop.Cli.Tests/Downloads/DownloadCommandHandlerTests.cs`: queue-mode regression keeps later downloads running when an earlier manifest repeats the same `fileId`.
...
- Assuming remote manifests provide unique identity fields and delegating duplicate handling to LINQ operators that throw framework exceptions.
