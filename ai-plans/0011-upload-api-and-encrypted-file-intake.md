...
- [x] A protected upload endpoint exists for encrypted file intake.
- [x] Upload requests require `Authorization: Bearer <admin-token>`.
- [x] The upload contract carries only encrypted file content and non-secret file metadata.
- [x] The API rejects uploads (400 Bad Request) unless recognized encryption metadata is present and internally
  consistent.
...
- [x] Upload validation rejects malformed lengths, invalid chunk counts, invalid salts, and inconsistent encryption
  metadata.
- [x] A successful upload returns the created file id and downstream-safe metadata (plaintext length, encrypted length,
  chunk count, encryption format version, algorithm id, chunk size) with no secrets, derived keys, or internal state.
- [x] Error responses must not expose secrets, key material, system paths, or internal validation details; errors are
  generic
  (400, 401, 413, 429) with minimal public message surface.
- [x] The upload endpoint enforces rate limiting or equivalent abuse protection to prevent high-volume upload spam.
- [x] Automated tests cover authorized upload success, unauthorized upload rejection, validation failures, and error
