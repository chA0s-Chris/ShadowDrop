...
internal sealed class QueueCreateCommandHandler
{
    ...
    public QueueCreateCommandHandler(CliConfigurationResolver configurationResolver,
    ...
    public async Task<Int32> ExecuteAsync(QueueCreateCommandOptions options, CancellationToken cancellationToken)
    {
        ...
        {
            await _standardError.WriteLineAsync("Specify the queue output path with --out.");
            return 1;
        }

        if (String.IsNullOrWhiteSpace(options.ShareToken))
        {
            await _standardError.WriteLineAsync("Specify a share token or share URL.");
            return 1;
        }
        ...
        {
            await _standardError.WriteLineAsync("--embed-secrets requires a share key (--share-key or --share-key-file).");
            return 1;
        }

        if (!options.EmbedSecrets && hasShareKey)
        {
            await _standardError.WriteLineAsync("A share key was supplied without --embed-secrets; pass --embed-secrets to embed it.");
            return 1;
        }
        ...
        try
        {
            configuration = _configurationResolver.Resolve(options.ServerUrlOverride, null);
        }
        ...
        {
            await _standardError.WriteLineAsync("Share token invalid or missing.");
            return 1;
        }

        if (serverUrl is null || (serverUrl.Scheme != Uri.UriSchemeHttp && serverUrl.Scheme != Uri.UriSchemeHttps))
        {
            await _standardError.WriteLineAsync("Server URL invalid or missing.");
            return 1;
        }
        ...
        try
        {
            var manifest = await new ShareManifestClient(_httpClient).GetAsync(serverUrl, shareToken, options.BearerToken, cancellationToken);
            ...
            await _standardOut.WriteLineAsync($"queue-file:{options.Out.FullName}");
            return 0;
        }
