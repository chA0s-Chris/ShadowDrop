...
public sealed class DownloadCommandHandlerTests
{
    ...
    public async Task InvokeAsync_ShouldContinueQueueProcessingAfterIndividualFailures()
    {
        ...
        var queuePath = fixture.CreateQueueFile(new()
        {
            ...
            ServerUrl = fixture.BaseAddress.ToString(),
            Entries =
            [
    ...
    public async Task InvokeAsync_ShouldDownloadSingleFileToOutputPath_WhenCliShareKeyOverridesKeyFile()
    {
        ...
        var exitCode = await CliApplication.InvokeAsync(
            [
                "download", share.ShareToken, "--server-url", fixture.BaseAddress.ToString(), "--share-key", upload.ShareKey,
    ...
    public async Task InvokeAsync_ShouldGuideInteractiveDownloadWithMaskedSecretsAndNoLeakage()
    {
        ...
        var exitCode = await CliApplication.InvokeAsync(["download", "--interactive", "--server-url", fixture.BaseAddress.ToString(), share.ShareToken],
    ...
    public async Task InvokeAsync_ShouldHonorBearerTokenArgument_ForProtectedShares()
    {
        ...
        var exitCode = await CliApplication.InvokeAsync(
            [
                "download", share.ShareToken, "--server-url", fixture.BaseAddress.ToString(), "--share-key", upload.ShareKey, "--bearer-token",
    ...
    public async Task InvokeAsync_ShouldMatchRequestedFileIdIgnoringGuidCasing()
    {
        ...
        var exitCode = await CliApplication.InvokeAsync(
            [
                "download", share.ShareToken, "--server-url", fixture.BaseAddress.ToString(), "--share-key", upload.ShareKey, "--file",
    ...
    public async Task InvokeAsync_ShouldRejectQueueFromAnUnsupportedVersionWithRecreationGuidance()
    {
        ...
        var standardOut = new StringWriter();
        var standardError = new StringWriter();
        var queuePath = Path.Combine(fixture.RootDirectory, "legacy.queue.json");

        // A version 1 queue, exactly as an earlier ShadowDrop CLI wrote it.
        await File.WriteAllTextAsync(queuePath,
                                     """
                                     {
                                       "shadowDrop": "1.0",
    ...
    // containment check in the downloader is retained as defense in depth behind this.
    [TestCase("../outside.bin", "outside.bin")]
    [TestCase("../downloads-evil/report.txt", "downloads-evil/report.txt")]
    public async Task InvokeAsync_ShouldRejectQueueOutputPathEscapingOutputRootBeforeDownloading(String outputPath, String escapedRelativePath)
    {
        var rootDirectory = Path.Combine(TestContext.CurrentContext.WorkDirectory, "artifacts", "download-command-handler-tests", Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            exitCode.Should().Be(1);
            File.Exists(Path.Combine(rootDirectory, escapedRelativePath.Replace('/', Path.DirectorySeparatorChar))).Should().BeFalse();
            standardError.ToString().Should().Contain("files[0].outputPath: The outputPath value must not contain '.' or '..' path segments.")
                         .And.Contain("The queue file is invalid.");
        }
        finally
        {
    ...
    private sealed class CliDownloadApiFactory : WebApplicationFactory<Program>, IAsyncDisposable
    {
        ...
        public async Task<UploadFixture> UploadFilesAsync(IReadOnlyList<String> filePaths, Boolean requireDownloadToken = false)
        {
            ...
            WriteConfig(BaseAddress.ToString(), BootstrapToken);
            var standardOut = new StringWriter();
            var standardError = new StringWriter();
